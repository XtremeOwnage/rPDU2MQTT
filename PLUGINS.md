...
**Dynamic assembly loading is out of scope, deliberately.** The GUI is TypeScript bundled into a single
`app.js` at build time, so a runtime DLL cannot add a page, a binding editor or a test button. The CRD is
generated from the config model, committed and pinned by a drift test, so config that isn't compiled in
cannot be expressed in an `RpduConfig`. In-tree modules deliver the whole ergonomic win without fighting
either. Revisit external loading later, scoped to destinations only.
