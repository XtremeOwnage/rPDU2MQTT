...
public static class FlowExport
{
    ...
    /// <summary>The HA unique_id of a tier's <c>out</c>-direction energy sensor ("energyflow_&lt;key&gt;_energy_out").
    /// Only published for a bidirectional node, where "energy" alone does not say which way.</summary>
    public static string EnergyOutUniqueId(string nodeId) => DeviceId(nodeId) + "_energy_out";
        ...
        bool includeEnergyToday = false)
    {
        ...
        // A bidirectional node (battery/grid) carries three: each direction named, and "energy" as the total
        // that passes through it either way. Naming only one direction leaves the other to be inferred from
        // a convention nobody can see on the screen — "Energy In" under "Energy exported to grid" reads as
        // its own opposite. The dashboard binds the named directions; the total is for the operator.
        if (includeEnergyIn)
        {
