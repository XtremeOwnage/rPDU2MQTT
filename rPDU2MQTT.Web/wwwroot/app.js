...
const FP_KINDS                     = [['outlet', 'Outlet'], ['switch', 'Switch'], ['fixture', 'Light fixture'], ['appliance', 'Appliance'], ['device', 'Device']];
...
function fpGlyph(kind        , r        )      {
  const g = svgEl('g', { class: 'fp-glyph' });
  const line = (x1        , y1        , x2        , y2        ) => g.appendChild(svgEl('line', { x1, y1, x2, y2 }));
  const k = r * 0.45;
...
function addFloorPlanSection(nav     , sections     ) {
  ...
  const freshItemId = (kind        ) => {
    const taken = new Set(itemsIn().map((p     ) => p.Id));
  ...
  const itemsNow = () => { const ids = new Set(roomsNow().map(r => r.Id)); return itemsIn().filter((p     ) => ids.has(p.Room)); };
  ...
  const fmt = (v                           ) => v == null ? 'no data' : formatMeasure(Math.round(v * (units() === 'W' ? 1 : 100)) / (units() === 'W' ? 1 : 100), units());
  ...
  const modeBtns                      = {};
  ([['view', 'View'], ['rooms', 'Rooms'], ['place', 'Place']]         ).forEach(([m, label]) => {
  ...
  const othersFor = (exclude     ) => [...roomsNow(), ...areasNow()].filter(s => s !== exclude && (s.Shape || []).length >= 3).map(s => s.Shape        );
  ...
  svg.addEventListener('pointerdown', (e     ) => {
    ...
    if (mode === 'rooms') {
      ...
      if (hit.corner != null && target) { gesture.kind = 'corner'; gesture.index = hit.corner; selectedCorner = hit.corner; }
        ...
        gesture.kind = 'move'; gesture.orig = (s?.Shape || []).map((q    ) => ({ ...q }));
      }
  ...
  const finishOutline = (poly      ) => {
    ...
    if (target && (target.Shape || []).length < 3 && (selection .type === drawKind)) { target.Shape = shape; }
      ...
      itemsIn().forEach((it     ) => { if (!it.Room && planShapeAt([{ Shape: shape }], { X: it.X, Y: it.Y })) it.Room = id; });
    }
    tool = 'select';
    changed();
    render();
  ...
  const drawSub = () => {
    ...
    if (mode === 'view') {
      ...
      subBar.appendChild(seg([['select', 'Select'], ['rect', 'Rectangle'], ['poly', 'Outline']], tool, v => { tool = v       ; draft = []; render(); }));
      subBar.appendChild(seg([['room', 'Room'], ['area', 'Area']], drawKind, v => { drawKind = v       ; render(); }, 'Draw a room, or an area that may span rooms.'));
  ...
  const drawSide = () => {
    ...
    if (selection && shapeOf(selection)) return drawShape(selection.type                   , shapeOf(selection));
    drawFloorSummary(fl);
  };
  ...
  const drawFloorSummary = (fl     ) => {
    ...
    const undrawn        = [];
    [...roomsNow().map(r => ['room', r]), ...areasNow().map(a => ['area', a])].forEach(([type, s]     ) => {
  ...
  const drawShape = (type                 , s     ) => {
    ...
    if (editing) {
      ...
      del.onclick = () => {
        ...
        if (type === 'room') { itemsIn().forEach((it     ) => { if (it.Room === s.Id) it.Room = ''; }); areasNow().forEach(a => { a.Rooms = (a.Rooms || []).filter((x        ) => x !== s.Id); }); }
        selection = null;
        changed();
        render();
      };
  ...
  const drawItem = (it     ) => {
    ...
    const lbl = el('input', { type: 'text', class: 'fp-name', value: it.Label || '', placeholder: 'e.g. Fridge, Desk outlet' })                    ;
  ...
  const openCircuit = (c         ) => {
    ...
    placed.forEach((it     ) => {
      const fl = floorsAll().find(x => ensure(x.floor, 'Rooms', []).some((r     ) => r.Id === it.Room));
      const b = el('button', { class: 'fp-list-row', type: 'button' },
  ...
  const floorSheet = () => {
    ...
    api('/api/plans/storage').then((r     ) => { limits.textContent = r.body?.ok ? `${r.body.limits} Kept in ${r.body.where}, never in the configuration. A large photo is shrunk here before it is sent.` : 'Plan storage is not reachable.'; }).catch(() => { limits.textContent = 'Plan storage is not reachable.'; });
    const file = el('input', { type: 'file', accept: 'image/png,image/jpeg,image/webp,image/svg+xml,.svg,image/*' })                    ;
    ...
    file.onchange = async () => {
      ...
      try {
        ...
        f.Image = b.id;
        imageFailed = '';
        const drawn = [...ensure(f, 'Rooms', []), ...ensure(f, 'Areas', [])].some((s     ) => (s.Shape || []).length >= 3) || itemsIn().some((it     ) => ensure(f, 'Rooms', []).some((r2     ) => r2.Id === it.Room));
        ...
        changed();
        render();
      } catch (e     ) { upStatus.textContent = e?.message || 'Could not read that image.'; }
    };
    ...
    del.onclick = () => {
      ...
      const ids = new Set(ensure(f, 'Rooms', []).map((r     ) => r.Id));
      const items = itemsIn();
