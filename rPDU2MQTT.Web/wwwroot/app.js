...
function trendsPage(nav     , sections     , spec            ) {
  ...
  const contextOf = (s      )              => within({ from: 2 * s.from - s.to, to: 2 * s.to - s.from });
  ...
  const loadPicked = async () => {
    ...
    const ask = (s      , st        ) => api(withInstance('/api/flow/series?'
