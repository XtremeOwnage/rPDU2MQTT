...
function historyControl(onChange: () => void): { row: HTMLElement, at: () => string, setNote: (t: string) => void } {
  const row = el('div', { class: 'ld-toolbar', style: { flexWrap: 'wrap', gap: '6px', margin: '0 0 8px' } });
  const input = el('input', { type: 'datetime-local', style: { width: 'auto' } }) as HTMLInputElement;
