...
export function addFlowSection(nav: any, sections: any) {
  ...
  const draw = (graph: any) => {
    ...
    const chains: { first: any; last: any; ways: string[] }[] = [];
    ...
    const cmp = (x: number, y: number) => x === y ? 0 : x - y;
    ...
    const tieBreak = (a: any, b: any) => (remainder(a.id) - remainder(b.id)) || (nodeValue(b.id) - nodeValue(a.id));
    ...
    const hierarchyOrder = () => {
      ...
      let next = 0;
      const primaryFeeder = (id: string) => ((cardIn[id] || []) as any[])
        .reduce((best: any, l: any) => (!best || (l.value || 0) > (best.value || 0) ? l : best), null)?.source;
      const feedsOn = (id: string) => ((cardOut[id] || []) as any[]).length > 0;
      const visit = (id: string) => {
        if (ord[id] != null) return;
        ord[id] = next++;
        ((cardOut[id] || []) as any[]).map((l: any) => l.target)
          .filter((t: string) => primaryFeeder(t) === id)
          .sort((a: string, b: string) => (Number(feedsOn(a)) - Number(feedsOn(b))) || (remainder(a) - remainder(b)) || (nodeValue(b) - nodeValue(a)))
          .forEach(visit);
      };
      folded.nodes.filter((n: any) => !((cardIn[n.id] || []) as any[]).length).map((n: any) => n.id)
        .sort((a: string, b: string) => nodeValue(b) - nodeValue(a)).forEach(visit);
      // Anything reached only through a loop.
      folded.nodes.forEach((n: any) => visit(n.id));
      const ordOf = (id: string) => byId[id]?.passthrough ? (ord[byId[id].to] ?? next) - 0.5 : (ord[id] ?? next);
      cols.forEach((cn: any[]) => cn && cn.sort((a: any, b: any) => ordOf(a.id) - ordOf(b.id)));
    };

    const orderOf = () => cols.map(cn => (cn || []).map((n: any) => n.id).join(',')).join('|');
    if (flowLayout === 'tiers') hierarchyOrder();
      ...
      cols.forEach((cn, c) => {
        if (c === 0) { if (sweep === 0) cn.sort((a: any, b: any) => tieBreak(a, b)); }
        else cn.sort((a: any, b: any) => cmp(bary(a.id), bary(b.id)) || tieBreak(a, b));
        placeColumn(cn, c);
      });
      ...
      for (let c = cols.length - 2; c >= 0; c--) {
        if (!cols[c]) continue;
        cols[c].sort((a: any, b: any) => cmp(bary(a.id), bary(b.id)) || cmp(obary(a.id), obary(b.id)) || tieBreak(a, b));
    ...
    const ribbonH = (l: any) => (l.known === false || l.value * pxPerUnit < 1.5) ? 1.5 : l.value * pxPerUnit;
    ...
    const relaxColumn = (c: number, side: 'out' | 'in') => {
      const cn = cols[c];
      ...
      cn.forEach((n: any) => {
        ...
        const theirs = links.map((a: any) => side === 'out' ? a.to : a.from);
        const mine = links.map((a: any) => side === 'out' ? a.from : a.to);
        const head = (xs: number[]) => Math.min(...xs);
    ...
    cols.forEach((cn: any[]) => { if (cn && cn.length) separate(cn); });
    ...
    bottom = Math.max(bottom, ...cols.filter(Boolean).flatMap((cn: any[]) => cn.map((n: any) => pos[n.id].y + rowOf(n.id))));
  ...
  const renderEditor = () => {
    ...
    [...cand.keys()].sort((a, b) => colMemo[b] - colMemo[a]).forEach(id => {
      ...
      const placed = outs.map((e: any) => colX[e.to]).filter((c: any) => typeof c === 'number');
    ...
    let dragNode: any = null, dragFrom: any = null, dragging = false, dragLine: any = null;
    ...
    const targetUnder = (cx: number, cy: number, self?: any) => { const hit: any = document.elementFromPoint(cx, cy); const gn = hit && hit.closest && hit.closest('g[data-id]'); return gn && gn.dataset.id !== (self ?? linkFrom) ? gn.dataset.id : null; };
    ...
    const setParent = (child: string, parent: string) => {
      ...
      const existing = links.filter((l: any) => l.To === child);
      if (existing.length && !confirm(
        `${nm(child)} is already fed by ${existing.map((l: any) => nm(l.From)).join(', ')}.\n\n`
        + `Replace that with ${nm(parent)}?\n\n`
        + `To keep both, cancel and drag ${nm(parent)}’s ● onto ${nm(child)} instead.`)) return;
      existing.forEach((l: any) => { const i = links.indexOf(l); if (i >= 0) links.splice(i, 1); });

      const derived = autoParent(child);
      links.push({ From: parent, To: child });
      toast(existing.length ? `${nm(parent)} now feeds ${nm(child)}, in place of ${existing.map((l: any) => nm(l.From)).join(', ')}.`
