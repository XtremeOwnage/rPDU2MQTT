// Which schema sections get a page of their own. Health, PlanStorage and Debug do not: the first two are
// deployment settings that belong with the volume they describe, in the config file or the chart's values,
// and Debug's switches are on Diagnostics with the state they are used to read.
...
for (const gone of ['Health', 'PlanStorage', 'Debug'])
...
for (const kept of ['MQTT', 'History', 'Diagnostics', 'Features', 'GUI'])
...
for (const section of ['Health', 'PlanStorage', 'Debug'])
...
console.log('pages: Health, PlanStorage and Debug have no page of their own, the schema still carries them, '
  + 'and Debug’s switches are on Diagnostics where an edit counts as an unsaved change');
process.exit(0);
