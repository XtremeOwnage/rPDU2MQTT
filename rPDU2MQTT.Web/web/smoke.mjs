...
function makeEl(tag = 'div') {
  const node = {
    ...
    // Listeners are recorded, not discarded, so a test can fire one — the hover card is only reachable
    // through mouseenter, and an untested tooltip is exactly the thing that rots unnoticed.
    _on: {},
