...
// Other pages have phone rules of their own; what must not happen is the dialog's being split across two
// blocks, where whichever is read first wins.
const phones = [...css.matchAll(/@media \(max-width: *560px\)\s*\{((?:[^{}]|\{[^{}]*\})*)\}/g)].map(m => m[1]);
const dialogBlocks = phones.filter(b => /\.sheet-panel|\.sheet-backdrop/.test(b));
if (!dialogBlocks.length) fail('no phone breakpoint carries the dialog rules');
if (dialogBlocks.length > 1) fail(`${dialogBlocks.length} phone breakpoints carry dialog rules — they are split, and whichever is read first wins`);
const p = dialogBlocks[0];
...
// total could not be reached without saving or discarding first. The page keeps the bar's measured height
// free at its foot.
if (!/body:has\(#savebar:not\(\.is-hidden\)\) main\s*\{[^}]*padding-bottom:\s*calc\(var\(--savebar-h/.test(css))
  fail('nothing keeps the foot of the page clear of the save bar');
if (!code.includes("'--savebar-h'")) fail('the save bar\'s height is never measured for the page to leave free');
console.log('phonedialog: the editing dialog scrolls both ways instead of clipping, its tables scroll '
  ...
  + 'inline, and one phone breakpoint gives the dialog the screen and drops the desktop control widths; the save bar leaves the foot of the page reachable');
